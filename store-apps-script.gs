...
// Menolak menandatangani lisensi jika secret masih default — mencegah menerbitkan
// token yang bisa dipalsukan siapa saja.
function assertSecretConfigured_(){
  if(!LICENSE_SECRET || LICENSE_SECRET === 'GANTI_SECRET_INI_DENGAN_ACAK_MIN_40_KARAKTER' || String(LICENSE_SECRET).length < 40){
    throw new Error('LICENSE_SECRET belum dikonfigurasi (min. 40 karakter acak). Set dulu di store-apps-script.gs sebelum menerbitkan lisensi.');
  }
}
function signLicense_(school, plan){
  assertSecretConfigured_();
...
// ====== MIDTRANS WEBHOOK ======
// Verifikasi signature_key Midtrans: sha512(order_id + status_code + gross_amount + ServerKey).
// Tanpa ini, siapa pun bisa mengirim POST palsu dan menerbitkan lisensi gratis.
function verifyMidtransSignature_(payload){
  var expected = String(payload.order_id || '')
               + String(payload.status_code || '')
               + String(payload.gross_amount || '')
               + MIDTRANS_SERVER_KEY;
  var raw = Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_512, expected, Utilities.Charset.UTF_8);
  var hex = raw.map(function(b){ return ('0' + (b & 0xff).toString(16)).slice(-2); }).join('');
  var got = String(payload.signature_key || '').trim().toLowerCase();
  // Perbandingan konstan-waktu.
  if(hex.length !== got.length) return false;
  var diff = 0;
  for(var i=0;i<hex.length;i++) diff |= (hex.charCodeAt(i) ^ got.charCodeAt(i));
  return diff === 0;
}
function handleMidtransWebhook_(payload){
  try{
    // Tolak jika server key belum dikonfigurasi (webhook tidak boleh aktif tanpa key).
    if(!MIDTRANS_SERVER_KEY || MIDTRANS_SERVER_KEY === 'SB-Mid-server-xxxxxx'){
      return ContentService.createTextOutput('Midtrans not configured').setMimeType(ContentService.MimeType.TEXT);
    }
    // WAJIB: verifikasi signature sebelum memproses apa pun.
    if(!verifyMidtransSignature_(payload)){
    ...
    var fraud = String(payload.fraud_status || 'accept');
    if((status === 'settlement' || status === 'capture') && fraud === 'accept'){
      ...
      if(row){
        ...
        // Cocokkan nominal yang dikonfirmasi Midtrans dengan nominal order (cegah manipulasi harga).
        if(Number(payload.gross_amount) !== Number(o.amount)){
          return ContentService.createTextOutput('Amount mismatch').setMimeType(ContentService.MimeType.TEXT);
        }
