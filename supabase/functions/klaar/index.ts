/**
 * Klaar App — Edge Function (Supabase / Deno)
 * Port dari master-apps-script-v5.gs. Multi-tenant, penyimpanan payload jsonb.
 *
 * Auth: token lisensi HMAC-SHA256 terbitan Klaar Store (byte-identik dgn Apps Script).
 * DB:   tabel licenses / databases(payload jsonb) / logs (lihat migrations/0001_init.sql).
 * Akses DB memakai service_role (melewati RLS); isolasi antar-sekolah oleh license_code
 * yang berasal dari token terverifikasi.
 *
 * Deploy: supabase functions deploy klaar --no-verify-jwt
 * Secret wajib: LICENSE_SECRET (samakan dgn store-apps-script.gs).
 */
...
const REQUIRE_SIGNED_LICENSE =
  (Deno.env.get("REQUIRE_SIGNED_LICENSE") ?? "true").toLowerCase() !== "false";
...
function unb64urlToStr(s: string): string {
...
async function hmacB64url(payloadStr: string): Promise<string> {
...
type LicResult =
  | { ok: true; school: string; plan: string; iat: string }
  | { ok: false; error: string };
async function verifyLicenseToken(token: string): Promise<LicResult> {
  try {
    token = String(token || "").trim();
    const parts = token.split(".");
    if (parts.length !== 3 || parts[0] !== LICENSE_PREFIX) {
      return { ok: false, error: "format token salah" };
    }
    const payloadStr = unb64urlToStr(parts[1]);
    const expected = await hmacB64url(payloadStr);
    const got = parts[2];
    if (expected.length !== got.length) {
      return { ok: false, error: "tanda tangan salah" };
    }
    let diff = 0;
    ...
    if (!payload || !payload.school) {
      return { ok: false, error: "payload tanpa school" };
    }
    return {
      ok: true,
...
async function log_(action: string, licenseCode: string, message: string) {
  try {
  ...
  } catch (_e) { /* logging tidak boleh menggagalkan request */ }
}

async function ensureLicenseRow(licenseCode: string, schoolName: string) {
  // Auto-provision: buat baris licenses jika belum ada (padanan ensureDemoLicense_).
  const { data } = await supabase.from("licenses").select("license_code")
    .eq("license_code", licenseCode).maybeSingle();
  if (!data) {
...
async function getRow(
  licenseCode: string,
): Promise<{ payload: DB; updated_at: string } | null> {
  const { data, error } = await supabase.from("databases")
    .select("payload, updated_at").eq("license_code", licenseCode).maybeSingle();
  if (error) throw error;
  return data as { payload: DB; updated_at: string } | null;
}

async function loadPayload(licenseCode: string): Promise<DB | null> {
...
async function upsertPayload(licenseCode: string, data: DB): Promise<DB> {
  const { data: out, error } = await supabase.from("databases")
    .upsert(
      { license_code: licenseCode, payload: data, updated_at: new Date().toISOString() },
      { onConflict: "license_code" },
    )
    .select("payload").single();
  if (error) throw error;
  return out.payload as DB;
}
...
async function savePayloadCAS(
  ...
  const { data: out, error } = await supabase.from("databases")
    ...
    .eq("license_code", licenseCode).eq("updated_at", prevUpdatedAt)
    .select("license_code");
  if (error) throw error;
...
function normalizeDb(data: DB, licenseCode: string): DB {
  ...
  data.attendanceRecords =
    data.attendanceRecords && typeof data.attendanceRecords === "object"
      ? data.attendanceRecords
      : {};
  data.attendanceRequests = Array.isArray(data.attendanceRequests)
    ? data.attendanceRequests
    : [];
  data.deviceRequests = Array.isArray(data.deviceRequests)
    ? data.deviceRequests
    : [];
  ...
  data.sentSlips = data.sentSlips && typeof data.sentSlips === "object"
    ? data.sentSlips
    : {};
  ...
  data.importHistory = Array.isArray(data.importHistory)
    ? data.importHistory
    : [];
...
function schoolNameFromLicense_local(_licenseCode: string): string {
  return "";
}
function defaultDb(licenseCode: string, schoolName: string): DB {
  return normalizeDb({
    version: "supabase",
    settings: {
...
// ====== Admin credential ======
function checkAdmin(db: DB, p: Params): boolean {
...
function norm_(s: unknown): string {
  return String(s || "").trim().toLowerCase().replace(/\s+/g, " ");
}
function flex_(s: unknown): string {
  return norm_(s).replace(/[^a-z0-9]/g, "");
}
function findEmployee(db: DB, login: string): DB | undefined {
  const q = norm_(login), f = flex_(login);
  const emps = db.employees || [];
  return emps.find((e: DB) =>
    flex_(e.nip) === f || flex_(e.loginName) === f || flex_(e.name) === f ||
    flex_(e.slipCode) === f || norm_(e.nip) === q || norm_(e.loginName) === q ||
    norm_(e.name) === q || norm_(e.slipCode) === q
  );
}
function checkEmployee(db: DB, p: Params): DB {
  const emp = findEmployee(db, p.nip || p.login || p.username || "");
  if (!emp) {
    throw new Error(
      "Karyawan tidak ditemukan. Coba login pakai NIP/Login/Nama/Kode yang tertulis di Admin.",
    );
  }
  if (["nonaktif", "arsip"].indexOf(String(emp.status || "Aktif").toLowerCase()) >= 0) {
  ...
  if (String(p.pinHash || "") !== effective) {
    throw new Error(
      "Username atau password salah. Untuk karyawan hasil import, default biasanya 1234.",
    );
  }
  return emp;
}
...
// ====== Geofence GPS (port) ======
function haversine_(lat1: number, lng1: number, lat2: number, lng2: number): number {
...
function activeLocations_(db: DB): DB[] {
  const r = (db && db.attendanceRules) || {};
  let locs: DB[] = Array.isArray(r.locations)
    ? r.locations.filter((l: DB) =>
      l && l.active !== false && Number(l.lat) && Number(l.lng)
    )
    : [];
...
// deno-lint-ignore no-explicit-any
function geofenceCheck_(db: DB, lat: number, lng: number, accuracy: number): any {
  const r = (db && db.attendanceRules) || {};
  const enforce = r.geofence !== false;
  const locs = activeLocations_(db);
  if (!locs.length) {
    return { ok: true, skip: true, distanceMeters: null, locationName: "" };
  }
  let best: { dist: number; loc: DB } | null = null;
  locs.forEach((l) => {
    const d = haversine_(Number(lat), Number(lng), Number(l.lat), Number(l.lng));
    if (!best || d < best.dist) best = { dist: d, loc: l };
  });
  best = best!;
  ...
  if (!Number(lat) || !Number(lng)) {
    return { ok: false, error: "Lokasi GPS tidak terbaca. Aktifkan GPS lalu coba lagi." };
  }
  if (accuracy && maxAcc && Number(accuracy) > maxAcc) {
    return {
      ok: false,
      error: "Akurasi GPS terlalu rendah (" + Math.round(accuracy) +
        "m, maksimal " + maxAcc + "m). Coba di area terbuka.",
    };
  }
  if (best.dist > radius) {
    return {
      ok: false,
      error: "Di luar radius lokasi: " + Math.round(best.dist) + "m dari " +
        (best.loc.name || "titik absensi") + " (maksimal " + radius + "m).",
    };
  }
  return info;
}
...
function timeToMin_(hhmm: string): number | null {
  const m = String(hhmm || "").match(/^(\d{1,2}):(\d{2})/);
  if (!m) return null;
  return Number(m[1]) * 60 + Number(m[2]);
}
function lateInfo_(db: DB, now: string) {
...
function normalizeLateRecord_(db: DB, r: DB): DB {
  if (!r || !r.checkInTime) return r;
  const st = String(r.status || "").toLowerCase();
  if (["izin", "sakit", "alpha", "pending"].indexOf(st) >= 0) return r;
...
function normalizeAllLateRecords_(db: DB): boolean {
  let changed = false;
  const recs = db.attendanceRecords || {};
  Object.keys(recs).forEach((date) => {
    Object.keys(recs[date] || {}).forEach((empId) => {
      const r = recs[date][empId];
      const before = JSON.stringify({ s: r?.status, l: r?.isLate, m: r?.lateMinutes, a: r?.lateAfter });
      normalizeLateRecord_(db, r);
      const after = JSON.stringify({ s: r?.status, l: r?.isLate, m: r?.lateMinutes, a: r?.lateAfter });
...
// ====== View karyawan (port) ======
function latestSlip_(db: DB, empId: string): DB | null {
  ...
  for (const m of months) {
    if (sent[m] && sent[m][empId] && locks[m] && locks[m].items) {
      const it = locks[m].items.find((x: DB) => x.id === empId);
      if (it) return Object.assign({ month: m }, it);
    }
  }
  return null;
}
function years_(join: string, imported: number): number {
...
function empView_(db: DB, emp: DB): DB {
  const d = today_();
  const recs = db.attendanceRecords || {};
  const todayRecord = normalizeLateRecord_(db, ((recs[d] || {})[emp.id] || null));
  const pos = (db.positions || []).find((x: DB) => x.id === emp.position) || {};
  const gr = (db.grades || []).find((x: DB) => x.id === emp.grade) || {};
  const hist = Object.keys(recs).sort().reverse().slice(0, 40)
    .map((date) => Object.assign({ date }, (recs[date] || {})[emp.id] || {}))
    .filter((x: DB) => x.status);
  ...
  return {
    ...
    history: hist,
    slip,
    todayRequest: (db.attendanceRequests || []).find((r: DB) =>
      r.employeeId === emp.id && r.date === d && r.status === "pending"
    ) || null,
    deviceStatus: "aktif",
  };
}
...
async function loadAdmin_(licenseCode: string, p: Params, tokenSchool: string) {
  ...
  if (!checkAdmin(data, p)) {
    ...
    return { ok: false, error: "Username/PIN admin salah." };
  }
  ...
  return { ok: true, license: { licenseCode, status: "active" }, data: out };
}

async function saveAdmin_(licenseCode: string, payloadStr: string, p: Params, tokenSchool: string) {
  await ensureLicenseRow(licenseCode, tokenSchool);
  ...
  if (!checkAdmin(existing || {}, p)) {
    ...
    return { ok: false, error: "Username/PIN admin salah. Sync ditolak." };
  }
  let parsed: DB;
  try {
    parsed = JSON.parse(payloadStr || "{}");
  } catch (err) {
    throw new Error("Payload admin bukan JSON valid: " + String((err as Error)?.message || err));
  }
  ...
  if (savedEmp !== expectEmp) {
    await log_("saveAdmin_verify_fail", licenseCode,
      "harusnya " + expectEmp + " karyawan, tersimpan " + savedEmp);
    return {
      ok: false,
      error: "Verifikasi server gagal: harusnya " + expectEmp +
        " karyawan, tersimpan " + savedEmp + ".",
    };
  }
...
async function changeAdminCredential_(licenseCode: string, p: Params) {
...
async function loadEmployee_(licenseCode: string, p: Params) {
...
async function employeeChangePassword_(licenseCode: string, p: Params) {
  return await withDB(licenseCode, (db) => {
    const emp = checkEmployee(db, p);
...
async function employeeCheckIn_(licenseCode: string, p: Params) {
  return await withDB(licenseCode, (db) => {
    const emp = checkEmployee(db, p);
    const d = today_();
    db.attendanceRecords = db.attendanceRecords || {};
    db.attendanceRecords[d] = db.attendanceRecords[d] || {};
    if (db.attendanceRecords[d][emp.id] && db.attendanceRecords[d][emp.id].checkInTime) {
    ...
    const r: DB = {
      ...
      message: late.isLate
        ? ("Check-in berhasil, status TELAT " + (late.minutes ? ("(" + late.minutes + " menit)") : "") + ".")
        : "Check-in berhasil",
      // selfie URL disimpan jika ada (diisi oleh uploadSelfie sebelumnya)
      checkInSelfieUrl: p.selfieUrl || "",
      checkInSelfieThumbUrl: p.selfieThumbUrl || "",
    };
    db.attendanceRecords[d][emp.id] = r;
    return { ok: true, record: r };
  }).catch((e) => ({ ok: false, error: String(e?.message || e) }));
}

async function employeeCheckOut_(licenseCode: string, p: Params) {
  return await withDB(licenseCode, (db) => {
    const emp = checkEmployee(db, p);
    const d = today_();
    db.attendanceRecords = db.attendanceRecords || {};
    db.attendanceRecords[d] = db.attendanceRecords[d] || {};
    const r = db.attendanceRecords[d][emp.id];
    if (!r || !r.checkInTime) throw new Error("Belum check-in.");
    if (r.checkOutTime) throw new Error("Sudah check-out.");
    ...
    // selfie checkout URL disimpan jika ada
...
async function employeeRequest_(licenseCode: string, p: Params) {
  return await withDB(licenseCode, (db) => {
    const emp = checkEmployee(db, p);
    db.attendanceRequests = db.attendanceRequests || [];
    const r = {
    ...
    return { ok: true, record: r, message: "Pengajuan dikirim" };
  }).catch((e) => ({ ok: false, error: String(e?.message || e) }));
}

// ====== Upload Selfie ke Supabase Storage ======
async function uploadSelfie_(licenseCode: string, p: Params): Promise<Record<string, unknown>> {
  try {
    const db = await loadPayload(licenseCode);
    ...
    if (uploadErr) {
      ...
      if (String(uploadErr.message).toLowerCase().includes("bucket") ||
          String(uploadErr.message).toLowerCase().includes("not found")) {
        throw new Error(
          "Bucket 'selfies' belum dibuat di Supabase Storage. " +
          "Buat lewat Dashboard: Storage > New Bucket > Name: selfies, Public: ON."
        );
      }
    ...
    await log_("uploadSelfie", licenseCode,
      `${emp.name} (${type}) ${d} - ${sizeKB}KB`);

    return { ok: true, selfieUrl, storagePath, sizeKB };
  } catch (e) {
...
async function getSelfieMeta_(
  ...
  try {
    ...
    const existing = await loadPayload(licenseCode);
    if (!checkAdmin(existing || {}, p)) {
      return { ok: false, error: "Admin auth gagal." };
    }
    ...
    let query: any = supabase
      ...
      .select(
        "id,employee_id,employee_name,attendance_date,type,selfie_url,lat,lng,accuracy,location_name,watermark_text,file_size_bytes,captured_at"
      )
    ...
    if (error) throw error;
    return { ok: true, selfies: data || [] };
  } catch (e) {
    return { ok: false, error: String((e as Error)?.message || e) };
  }
}

async function archiveEmployee_(licenseCode: string, empId: string) {
  return await withDB(licenseCode, (db) => {
    const emp = (db.employees || []).find((e: DB) => e.id === empId);
    ...
    return { ok: true, message: "Karyawan diarsipkan" };
  }).catch((e) => ({ ok: false, error: String(e?.message || e) }));
}

async function deleteEmployee_(licenseCode: string, empId: string) {
  return await withDB(licenseCode, (db) => {
    db._deletedEmployees = db._deletedEmployees || [];
    if (empId && db._deletedEmployees.indexOf(empId) < 0) db._deletedEmployees.push(empId);
    db.employees = (db.employees || []).filter((e: DB) => e.id !== empId);
    Object.keys(db.attendanceRecords || {}).forEach((d) => {
      if (db.attendanceRecords[d]) delete db.attendanceRecords[d][empId];
    });
    db.attendanceRequests = (db.attendanceRequests || []).filter((r: DB) => r.employeeId !== empId);
    db.deviceRequests = (db.deviceRequests || []).filter((r: DB) => r.employeeId !== empId);
    Object.keys(db.locks || {}).forEach((m) => {
      if (db.locks[m] && db.locks[m].items) {
        db.locks[m].items = db.locks[m].items.filter((x: DB) => x.id !== empId);
      }
    });
...
async function approveLateAsPresent_(licenseCode: string, empId: string, date: string) {
  return await withDB(licenseCode, (db) => {
    const d = date || today_();
    db.attendanceRecords = db.attendanceRecords || {};
    db.attendanceRecords[d] = db.attendanceRecords[d] || {};
...
async function repairDatabase_(licenseCode: string, tokenSchool: string) {
  await ensureLicenseRow(licenseCode, tokenSchool);
  const row = await getRow(licenseCode);
  if (!row) {
...
async function syncCheck_(licenseCode: string) {
  const row = await getRow(licenseCode);
  if (!row) return { ok: true, exists: false, employees: 0, serverUpdatedAt: "" };
  const d = row.payload || {};
  return {
    ok: true,
    exists: true,
...
async function route(p: Params) {
  const action = p.action || "loadAdmin";
  const licenseCode = String(p.licenseCode || DEFAULT_LICENSE).trim();

  if (action === "ping" || action === "health") {
    return { ok: true, app: "Klaar", version: "supabase", licenseCode, time: new Date().toISOString() };
  }
  ...
  if (REQUIRE_SIGNED_LICENSE) {
    if (!LICENSE_SECRET || LICENSE_SECRET === "GANTI_SECRET_INI_DENGAN_ACAK_MIN_40_KARAKTER" || LICENSE_SECRET.length < 40) {
      ...
      return { ok: false, error: "Server belum dikonfigurasi: LICENSE_SECRET wajib diisi (min. 40 karakter). Hubungi penjual." };
    }
    ...
    const { data: licRow } = await supabase.from("licenses")
      .select("status").eq("license_code", licenseCode).maybeSingle();
    if (licRow && String(licRow.status || "active") !== "active") {
      await log_("license_suspended", licenseCode, "status=" + licRow.status);
      return { ok: false, error: "Lisensi dinonaktifkan. Hubungi penjual." };
    }
  }

  switch (action) {
    case "repairDatabase":
      return await repairDatabase_(licenseCode, tokenSchool);
    case "syncCheck":
    ...
    case "archiveEmployee":
      return await archiveEmployee_(licenseCode, p.employeeId);
    case "deleteEmployee":
      return await deleteEmployee_(licenseCode, p.employeeId);
    case "approveLateAsPresent":
      return await approveLateAsPresent_(licenseCode, p.employeeId, p.date);
    default:
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    return jsonResponse({ ok: false, error: String((err as Error)?.message || err) });
  }
});
