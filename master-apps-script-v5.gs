...
function doPost(e){
  ...
  let out;
  try { out = route_(p, true) || {ok:true}; }
  catch(err){
    out = {ok:false, error:String(err && err.message || err)};
    log_('post_error', String(p.licenseCode||''), out.error);
  }
  // Kembalikan status asli ke frontend (via postMessage) agar kegagalan sync
  // (PIN salah / lisensi invalid / payload rusak) tidak dikira berhasil.
...
function route_(p, post){
  ...
  if(REQUIRE_SIGNED_LICENSE){
    // Guard konfigurasi: menolak jalan jika secret masih default. Kalau dibiarkan default
    // (dan Klaar Store juga default), token bisa dipalsukan siapa saja.
    if(!LICENSE_SECRET || LICENSE_SECRET === 'GANTI_SECRET_INI_DENGAN_ACAK_MIN_40_KARAKTER' || String(LICENSE_SECRET).length < 40){
      log_('config_error', licenseCode, 'LICENSE_SECRET belum dikonfigurasi');
      return {ok:false, error:'Server belum dikonfigurasi: LICENSE_SECRET wajib diisi (min. 40 karakter acak) dan sama dengan Klaar Store. Hubungi penjual.'};
    }
...
function loadAdmin_(licenseCode, p){
  ...
  // Jangan kirim hash kredensial admin balik ke browser di setiap load (kurangi paparan/replay).
  // Server tetap sumber kebenaran: saveAdmin_ mempertahankan hash yang tersimpan, dan
  // changeAdminCredential_ mengubahnya. Frontend tidak butuh nilai ini untuk mode server.
...
function checkEmployee_(db,p){
  ...
  // Jangan pernah izinkan login tanpa PIN. Kalau PIN belum di-set, pakai default (sama dengan
  // hash '1234') supaya karyawan tetap bisa masuk pertama kali, tapi TIDAK boleh lolos tanpa password.
  const effective = expected || DEFAULT_ADMIN_HASH; // DEFAULT_ADMIN_HASH = sha256('1234'+SALT), sama dgn DEFAULT_EMP_HASH
  if(String(p.pinHash||'') !== effective) throw new Error('Username atau password salah. Untuk karyawan hasil import, default biasanya 1234.');
  return emp;
}
